...
@Component({
  selector: 'app-mod-actividad',
  templateUrl: './mod-actividad.component.html',
  styleUrls: ['./mod-actividad.component.css']
})
export class ModActividadComponent implements OnInit {
  ...
  putActividades(){
    ...
    if(this.nomFormControl.valid ) {
      this.httpClient.put('http://159.65.222.132:804/api/Actividades/'+this.data.id,{id: this.data.id, nombre: this.ActividadesDTO.nombre, activo: op} ).subscribe(data => {
